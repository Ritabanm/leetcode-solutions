...
WITH events AS (
    ...
    SELECT employee_id,
           ...
           -1 as event_status
    FROM Tasks
    ORDER BY employee_id, event_time, event_status
)
...
FROM (
    SELECT
        employee_id,
        CASE WHEN event_status > 0 AND @event_count = 0 THEN @start_time := event_time
             ELSE @start_time END AS start_time,
        @event_count := @event_count + event_status AS event_count,
        CASE WHEN event_status < 0 AND @event_count = 0 THEN event_time
             ELSE @start_time END AS end_time
    FROM events
    JOIN (SELECT @event_count := 0, @start_time := 0) e
) e1
